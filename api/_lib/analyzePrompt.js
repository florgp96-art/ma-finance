...
- Para cuotas: completar cuotas_total y cuota_numero SOLO cuando el resumen indica explícitamente que es una compra financiada en cuotas (aparece la palabra "cuota"/"cuotas", o un formato tipo "03/12" pegado a una compra puntual, ej. "NETFLIX 03/12"). Si no hay ninguna señal de eso, dejar cuotas_total: 1, cuota_numero: 1.
  ...
  Formato AMEX "Facturación acelerada de NN cuotas" / "Primer cuota facturada MM/AA": esto significa que las cuotas que quedaban pendientes de un plan se juntaron y se cobraron todas de una sola vez en este resumen (el banco "aceleró" el cobro) — NO es el inicio de un plan nuevo de NN cuotas, y el "MM/AA" es el mes/año en que se facturó la primera cuota (no un "cuota MM de AA"). Como este cobro cierra el plan y no quedan más cuotas por venir, va con cuotas_total: 1, cuota_numero: 1 (no proyectar cuotas futuras). Esta línea y la siguiente ("Primer cuota facturada...") son parte de LA MISMA transacción que el comercio de la línea de arriba (ej. "MERPAGO*LPBS") — nunca generes una transacción aparte llamada "Cuota" o similar a partir de estas líneas descriptivas.
...
- COMPRA / VENTA DE TÍTULOS, VALORES o BONOS (ej. "COMP. TITULOS / VAL. AL30", "VENTA DE TITULOS / VALORES - AL30": así se hace el dólar MEP) → Inversiones, con la misma regla de tipo que la línea anterior: "ingreso" si es un crédito, "gasto" si es un débito, nunca "neutro".
