...
export default async function handler(req, res) {
  ...
  const response = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: 'user',
        content: `Extraé las transacciones de este extracto bancario argentino de la tarjeta "${cardName}". Devolvé SOLO JSON válido con esta estructura exacta:
...
- NETFLIX/CANVA/APPLE.COM/APPLE.COM BILL → Suscripciones
...
- es_credito: true solo para pagos recibidos (SU PAGO)
...
${pdfText}`
  ...
  // Minificar el JSON en el servidor antes de devolver
  try {
    ...
    if (textBlock?.text) {
      ...
      // Devolver JSON minificado
      data.content[0].text = JSON.stringify(parsed)
    }
  } catch (e) {
    // Si falla el minificado, devolver igual y que el frontend maneje el error
